...
if [ "$str1" = "5.4" ];then
  ...
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver61}0_uefi-gpt_dev_dscaodocker.img.gz
 else
  ...
  if [ ! $ver66 ]; then
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver66}0_dev_dscaodocker.img.gz
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver66}0_uefi-gpt_dev_dscaodocker.img.gz
 else
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver66}_dev_dscaodocker.img.gz
  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver66}_uefi-gpt_dev_dscaodocker.img.gz
   fi
fi
...
#!/bin/bash
# https://github.com/dscao/Actions-OpenWrt-docker-x86
# Actions-OpenWrt-docker-x86 By dscao 20230124
...
if [ -s  "/tmp/cloud_ts_version" ]; then
	...
	DEV_URL=https://github.com/dscao/Actions-OpenWrt-docker-x86/releases/download/${new_version}/openwrt_x86-64-${new_version}_dev_dscaodocker.img.gz
	DEV_UEFI_URL=https://github.com/dscao/Actions-OpenWrt-docker-x86/releases/download/${new_version}/openwrt_x86-64-${new_version}_uefi-gpt_dev_dscaodocker.img.gz
	openwrt_dev=https://github.com/dscao/Actions-OpenWrt-docker-x86/releases/download/${new_version}/openwrt_dev.md5
	openwrt_dev_uefi=https://github.com/dscao/Actions-OpenWrt-docker-x86/releases/download/${new_version}/openwrt_dev_uefi.md5
else
	echo "请检测网络或重试！"
	exit 1
fi
...
#!/bin/bash
# https://github.com/dscao/Actions-OpenWrt-docker-x86
# Actions-OpenWrt-docker-x86 By dscao 20210505
#path=$(dirname $(readlink -f $0))
# cd ${path}
#检测准备
if [ ! -f  "/etc/dscao_version" ]; then
